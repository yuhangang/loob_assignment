...
type CartRepository interface {
	GetCart(ctx context.Context, userID, countryID string) ([]CartItem, error)
...
func (s *Service) GetCart(ctx context.Context, countryCode, userID string) (CartResponse, error) {
	if strings.TrimSpace(userID) == "" {
		return CartResponse{}, ErrUserRequired
	}
	items, err := s.repo.GetCart(ctx, userID, countryCode)
	if err != nil {
		return CartResponse{}, err
	}
	return toCartResponse(countryCode, userID, items), nil
}
...
func (s *Service) UpsertItem(ctx context.Context, countryCode string, req CartItemRequest) (CartResponse, error) {
	...
	return s.GetCart(ctx, countryCode, req.UserID)
}
...
func (s *Service) UpdateItem(ctx context.Context, countryCode string, itemID int64, req CartItemUpdateRequest) (CartResponse, error) {
	...
	return s.GetCart(ctx, countryCode, cartReq.UserID)
}
...
func (s *Service) RemoveItem(ctx context.Context, countryCode, userID string, itemID int64) (CartResponse, error) {
	...
	return s.GetCart(ctx, countryCode, userID)
}
...
func toCartResponse(countryID, userID string, items []CartItem) CartResponse {
	resp := CartResponse{
		UserID:    userID,
		CountryID: countryID,
