...
erDiagram
    ...
    STORES ||--o{ CART_ITEMS : "contains active cart lines"
    ...
    MENU_ITEMS ||--o{ CART_ITEMS : "selected in"
    MENU_ITEMS ||--o{ ORDER_INTENT_ITEMS : "snapshotted as"

    CUSTOMIZATION_GROUPS ||--o{ CUSTOMIZATION_OPTIONS : "contains"
    CUSTOMIZATION_OPTIONS ||--o{ ORDER_INTENT_ITEM_OPTIONS : "snapshotted as"
...
CREATE TABLE categories (
    ...
    INDEX idx_categories_brand_display (is_active, brand_id, display_order)
);
...
CREATE TABLE vouchers (
    ...
    stacking_group VARCHAR(32) NOT NULL DEFAULT 'CART_DISCOUNT', -- One voucher per group in a stack
    stacking_priority INT NOT NULL DEFAULT 100, -- Lower priority applies first
    exclusive BOOLEAN NOT NULL DEFAULT false, -- Cannot combine with other vouchers
    combinable_with_groups JSON NULL, -- Optional allow-list of compatible stacking groups
...
Voucher stacking is represented by `order_intent_vouchers`. `order_intents.voucher_code` remains a backward-compatible first-code display field, while the join table is the source of truth for applied voucher breakdown, support lookups, refunds, and payment-capture redemption effects.
...
CREATE TABLE order_intent_vouchers (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
